...
**Research and Development Intern** \| *Aug 2025 – Present* \| *Advisor: Dr. Neil Smith*
...
* Built a production-grade **retrieval-augmented generation (RAG)** chatbot using Azure OpenAI, reducing customer support response time from days to under 20 seconds.
* Designed a punctuation-aware Mandarin tokenizer to improve retrieval accuracy for ambiguous queries.
* Automated ingestion and update pipelines to ensure continuous refresh of the search index.
* Collaborated with frontend engineers to deploy a polished UI.
* Selected for **Best Technical Intern Project**.
...
* Designed controlled experiments to measure **Performance Gain Recovered (PGR)** in cross-model supervision.
* Benchmarked GPT-4.x and GPT-5 models under noisy or weak supervision.
* Implemented an evaluation pipeline with async API calls, structured JSON logging, and reproducible metrics.
* Found that structure-preserving feedback improves performance over direct answer-level supervision.
...
* Built CNN–MLP and **ClimaX** models for temperature and precipitation forecasting.
* Designed a custom area-weighted loss function to reduce regional error in high-variance latitudes.
* Achieved **Top 2 / 83 teams**, with a 46% reduction in RMSE from baseline.

---

### Tumor Detection with U-Net

*Jan 2023 – Jun 2023*

* Implemented and trained a U-Net model for CT tumor segmentation using TensorFlow/Keras.
* Preprocessed 100+ medical scans (normalization, mask parsing, augmentation).
* Improved mean Dice score through depth tuning, dropout adjustment, and spatial augmentations.
